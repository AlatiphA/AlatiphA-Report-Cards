// AlatiphA Report Cards — app-27.js
...
function showView(name) {
  ...
  if (name === 'setup') refreshHeadTeacherSelect();
...
function refreshProfileMenu() {
  ...
  if (!FIREBASE_ENABLED) {
    ...
    accountLine.textContent = 'Continuing as guest (no account)';
    logoutBtn.classList.add('hidden');
  }
}
...
function initAuth() {
  ...
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      hideAuthGate();
      initLockScreen();
      if (!appStarted) { appStarted = true; proceedToApp(); } else { refreshProfileMenu(); }
    } else {
      currentUid = null;
