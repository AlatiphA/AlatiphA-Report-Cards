// AlatiphA Report Cards — app-9.js
...
function drawReportPage(doc, result, settings, positions, numOnRoll, classInfo, studentRemarks) {
  ...
  y += 6;
  doc.setFont('helvetica', 'bold');
  doc.text(`Class Position: ${result.position ? ordinal(result.position) : '-'}`, left, y);
  doc.text(`Total Score: ${result.totalSum}`, left + 70, y);
  doc.text(`Aggregate: ${result.aggregate !== null ? result.aggregate : '-'}`, right - 35, y);
  doc.setFont('helvetica', 'normal');
