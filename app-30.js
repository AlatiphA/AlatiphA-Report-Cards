// AlatiphA Report Cards — app-30.js
...
// currentUid is null unless Firebase Auth is enabled AND a teacher is
// signed in. When null, keys resolve exactly as before (unnamespaced) —
// so existing local-only installs are completely unaffected. Once a
// teacher signs in, every KEYS.* reference below automatically switches
// to that teacher's own namespaced storage, with no other code changes
// needed anywhere else in the app.
let currentUid = null;
function ns(base) { return currentUid ? `${base}__${currentUid}` : base; }
...
function showView(name) {
  ...
  document.getElementById('brandText').textContent = name === 'home' ? 'AlatiphA Report Cards' : sectionTitle(name);
...
function sectionTitle(name) {
  ...
  return titles[name] || 'AlatiphA Report Cards';
}
...
function refreshProfileMenu() {
  ...
  } else if (currentUid && firebase.auth().currentUser) {
    accountLine.textContent = firebase.auth().currentUser.email;
...
document.getElementById('importBackupInput').addEventListener('change', e => {
  ...
  reader.onload = () => {
    ...
    if (!parsed || !parsed.data) {
      alert('That file does not look like an AlatiphA Report Cards backup.');
      e.target.value = '';
      return;
    }
...
/* ---------- Teacher accounts (Firebase Auth) — optional, off until firebase-config.js has real values ---------- */
...
// One-time copy of pre-accounts local data into a newly-signed-in
// teacher's own namespaced storage, so someone who already used the app
// before creating an account doesn't see everything appear empty.
function migrateLegacyDataIfNeeded() {
  if (!currentUid) return;
  const alreadyMigrated = localStorage.getItem(ns('arc_migrated'));
  ...
  legacyKeys.forEach(base => {
    ...
    const namespacedKey = ns(base);
    if (legacyValue !== null && localStorage.getItem(namespacedKey) === null) {
      localStorage.setItem(namespacedKey, legacyValue);
      copiedAnything = true;
    }
  });
  localStorage.setItem(ns('arc_migrated'), '1');
  if (copiedAnything) {
    alert('Your existing data on this device has been linked to your account.');
  }
}

/* ---------- Cloud Sync (Firestore) — syncs text data across devices for
   the signed-in teacher. Photos, signatures, and the school logo stay
   local to each device for now (they're base64 images, and Firestore
   documents cap at 1MB — full image sync needs Firebase Storage, a
   separate future upgrade). ---------- */
const LAST_SYNCED_KEY = 'arc_last_synced';

function syncableFields() {
  return [
    { field: 'settings', key: KEYS.settings },
    { field: 'classes', key: KEYS.classes },
    { field: 'subjects', key: KEYS.subjects },
    { field: 'students', key: KEYS.students },
    { field: 'grades', key: KEYS.grades },
    { field: 'remarks', key: KEYS.remarks },
    { field: 'staff', key: KEYS.staff }
  ];
}
...
function scheduleCloudPush(rawKey) {
  if (!FIREBASE_ENABLED || !currentUid) return;
...
function pushFieldToCloud(match) {
  ...
  firebase.firestore().collection('teachers').doc(currentUid)
    .set({ [match.field]: cleaned, updatedAt: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true })
    .then(() => setLastSyncedNow())
    .catch(err => console.error('Cloud sync failed for', match.field, err));
}

// Pulls the teacher's cloud document down. If it doesn't exist yet
// (first-ever sync for this account), pushes whatever's local up
// instead, so that becomes the starting cloud copy.
function pullCloudData() {
  if (!FIREBASE_ENABLED || !currentUid) return Promise.resolve();
  return firebase.firestore().collection('teachers').doc(currentUid).get().then(doc => {
    if (!doc.exists) {
    ...
    syncableFields().forEach(f => {
      ...
      const merged = mergeCloudWithLocalImages(f.field, data[f.field]);
...
function renderCloudSyncStatus() {
  ...
  if (!currentUid) {
    wrap.innerHTML = '<p class="hint">You are using guest mode — data stays on this device only. Log in with an account to enable cloud sync.</p>';
    btn.classList.add('hidden');
    return;
  }
  const last = localStorage.getItem(LAST_SYNCED_KEY);
  const lastText = last ? new Date(Number(last)).toLocaleString() : 'never';
  wrap.innerHTML = `<p class="hint">Signed in as ${escapeHtml(firebase.auth().currentUser.email)}. Last synced: ${lastText}. Photos, signatures, and the school logo stay local to each device — only text data syncs.</p>`;
...
function showAuthGate() {
  document.documentElement.classList.add('authing');
}
...
function renderAuthForm() {
  document.getElementById('authHeading').textContent = authMode === 'login' ? 'Teacher Sign In' : 'Create Teacher Account';
...
function initAuth() {
  ...
  firebase.auth().onAuthStateChanged(user => {
    if (user) {
      ...
      showSyncingMessage();
      pullCloudData().catch(err => console.error('Initial sync failed:', err)).finally(() => {
        hideSyncingMessage();
        hideAuthGate();
        initLockScreen();
