// AlatiphA Report Cards — app-18.js
...
document.getElementById('importBackupInput').addEventListener('change', e => {
  ...
  reader.onload = () => {
    ...
    const ok = confirm('This will replace ALL current classes, students, subjects, grades and remarks with the contents of this backup. This cannot be undone. Continue?');
...
function renderClassStatistics() {
  ...
  if (!stats.studentsWithResults) {
    wrap.innerHTML = '<h3 class="subsection-title">Class Statistics</h3><p class="empty">No grades entered yet for this class.</p>';
    return;
  }

  let html = '<h3 class="subsection-title">Class Statistics</h3>';
  html += `<p class="hint">${stats.studentsWithResults} of ${stats.totalStudents} student(s) have results · `
    ...
    + '</p>';
  html += '<div class="table-scroll"><table class="grades-table"><thead><tr>'
    + '<th class="name-col">Subject</th><th>Avg</th><th>Highest</th><th>Lowest</th><th>Pass %</th></tr></thead><tbody>';
  stats.subjectStats.forEach(s => {
    if (!s.count) {
      html += `<tr><td class="name-col">${escapeHtml(s.subject.name)}</td><td colspan="4">No data</td></tr>`;
      return;
    }
    html += `<tr><td class="name-col">${escapeHtml(s.subject.name)}</td>`
