// AlatiphA Report Cards — app-1.js
...
function renderGradesTable() {
  ...
  students.forEach(st => {
    ...
    subjects.forEach(sub => {
      ...
      html += `<td><input type="number" min="0" max="100" data-student="${st.id}" data-subject="${sub.id}" value="${val}"></td>`;
    });
    html += '</tr>';
  });
...
document.getElementById('saveGradesBtn').addEventListener('click', () => {
  ...
  document.querySelectorAll('#gradesTableWrap input').forEach(input => {
    ...
    if (raw === '') { delete classGrades[studentId][subjectId]; }
    else {
      let n = Math.max(0, Math.min(100, Number(raw)));
      classGrades[studentId][subjectId] = n;
    }
  });
...
function drawReportPage(doc, result, settings) {
  ...
  if (settings.logo) {
    try { doc.addImage(settings.logo, 'PNG', 15, y, 22, 22); } catch (e) {}
  }
  doc.setFontSize(15);
  doc.setFont(undefined, 'bold');
  doc.text(settings.schoolName || 'School Name', pageWidth / 2, y + 8, { align: 'center' });
  doc.setFontSize(10);
  doc.setFont(undefined, 'normal');
  if (settings.address) doc.text(settings.address, pageWidth / 2, y + 14, { align: 'center' });
  doc.setFontSize(11);
  doc.setFont(undefined, 'bold');
  doc.text('TERMINAL REPORT', pageWidth / 2, y + 22, { align: 'center' });
