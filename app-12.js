// AlatiphA Report Cards — app-12.js
...
function drawReportPage(doc, result, settings, positions, numOnRoll, classInfo, studentRemarks) {
  ...
  if (simple) {
    // Simple layout: no Grade/Aggregate — just position, total and an
    // overall remark, more appropriate for Basic/KG-level reporting.
    field('Class Position', result.position ? ordinal(result.position) : '-', left, y);
    field('Total Score', result.totalSum, left + 70, y);
    field('Remark', result.overallRemark || '-', right - 35, y);
  } else {
    field('Class Position', result.position ? ordinal(result.position) : '-', left, y);
    field('Total Score', result.totalSum, left + 70, y);
  ...
  // Simple: Subject | Total | Remark — column widths always sum to the
  // full printable width so the table fills the page edge-to-edge.
  const colW = simple ? [90, 30, 60] : [46, 19, 19, 19, 15, 18, 44];
  const colX = [left];
  colW.forEach(w => colX.push(colX[colX.length - 1] + w));
  const headers = simple ? ['Subject', 'Total', 'Remark'] : ['Subject', 'Class', 'Exam', 'Total', 'Grade', 'Position', 'Remark'];
  const rowH = 8;
  const lastCol = colW.length;
  const centerCols = simple ? [1] : [1, 2, 3, 4, 5]; // numeric columns center; Subject/Remark stay left-aligned
  ...
  result.entries.forEach(en => {
    ...
    if (simple) {
      ...
      cellText(en.remark, 2, y + 5.5, false);
      doc.setTextColor(0, 0, 0);
    } else {
      cellText(String(en.subject.name), 0, y + 5.5, false);
