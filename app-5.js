// AlatiphA Report Cards — app-5.js
...
function computeClassResults(classId, term, year) {
  ...
  const results = students.map(st => {
    ...
    return {
      student: st, entries, totalSum, avg,
      overallGrade: entries.length ? getGradeFor(avg) : null,
...
function drawReportPage(doc, result, settings, positions, numOnRoll, classInfo, studentRemarks) {
  ...
  y += 8;
  // Results table: Subject | Class /50 | Exam /50 | Total | Grade | Position | Remark
  const colW = [42, 18, 18, 18, 14, 16, 34];
  const colX = [left];
  ...
  result.entries.forEach(en => {
    ...
    doc.text(String(en.classScaled), colX[1] + 2, y + 5.5);
    doc.text(String(en.examScaled), colX[2] + 2, y + 5.5);
    doc.text(String(en.total), colX[3] + 2, y + 5.5);
    if (weak) doc.setTextColor(RED_INK[0], RED_INK[1], RED_INK[2]);
    doc.text(String(en.grade), colX[4] + 2, y + 5.5);
    doc.text(pos ? ordinal(pos.position) : '-', colX[5] + 2, y + 5.5);
    doc.text(en.remark, colX[6] + 2, y + 5.5);
    doc.setTextColor(0, 0, 0);
    y += rowH;
  });
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.text('Overall', colX[0] + 2, y + 5.5);
  doc.text(String(result.totalSum), colX[3] + 2, y + 5.5);
  doc.text(result.overallGrade ? String(result.overallGrade) : '-', colX[4] + 2, y + 5.5);
  doc.text(result.position ? ordinal(result.position) : '-', colX[5] + 2, y + 5.5);
  doc.text(result.overallRemark || '-', colX[6] + 2, y + 5.5);
  y += rowH + 8;
